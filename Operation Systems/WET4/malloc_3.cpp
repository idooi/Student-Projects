...
void debug(){
    std::cout<<"\nfreeBlocks: "<< _num_free_blocks() << " freeBytes: " << _num_free_bytes()
             <<"  allocBlocks: " << _num_allocated_blocks() << " allocBytes: "<< _num_allocated_bytes()
             <<" metaBytes: " << _num_meta_data_bytes() << std::endl;
    cout<< "Heap: " <<endl;
    MallocMetadata* curr = memList;
    ...
    if (prev != lastMMap){
        cout << "last mmmmap is fucked" <<endl;
    }
}*/
...
void sfree(void* p){
    ...
    // on heap
    if(metadata->isFree) return;// stupid double free kid
    merge(metadata);//merge if needed
